...
namespace CommonTools
{
    public static class HttpHelper
    {
        ...
        /// 新浪开发者id
        /// </summary>
        private const string Key = "sina_appid";
        ...
        /// <returns></returns>
        public static string Shorten(string sourceUrl)
        {
            string type = "xml"; //返回xml格式 也可以是json
            string target = "https://api.t.sina.com.cn/short_url/shorten." + type;

            string appid = GetAppConfig(Key);
            string request = String.Format(target + "?source={0}&url_long={1}", appid, sourceUrl);

            try
            {
                string xml = GetHttpResponse(request);
                return XmlHelper.ReadShortenNodeInnerText(xml, "/urls/url/url_short");
            }
            catch (Exception)
            {
                return null;
        ...
        private static string GetAppConfig(string key)
        {
            try
            {
                string path = HttpContext.Current.Server.MapPath("/bin/app.config");
        ...
        /// <returns></returns>
        public static string PostCreate(string url, Dictionary<string, string> paramterList)
        {
            var formData = new StringBuilder();
            if (paramterList == null || paramterList.Count <= 0) return PostCreate(url, formData.ToString());
        ...
        public static string PostCreate(string url, string formData)
        {
            string responseText = "";
            try
            {
                ...
                var myreader = new StreamReader(response.GetResponseStream(), Encoding.UTF8);
        ...
        public static string GetHttpResponse(string url)
        {
            ...
            try
            {
                var myHttpWebResponse1 = (HttpWebResponse)myHttpWebRequest.GetResponse();
                //设置页面的编码模式
                var utf8 = Encoding.UTF8;
                var streamResponse = myHttpWebResponse1.GetResponseStream();
        ...
        /// 生成静态页面
        /// </summary>
        /// <param name="url">页面http://全路径</param>
        /// <param name="domain">生成到域名文件夹下名称,使用常量DomainConst</param>
        /// <param name="pageName">页面名称,不需要加html</param>
        /// <param name="encoding">编码</param>
        public static void CreateStaticHtml(string url, string domain, string pageName, Encoding encoding)
        {
            var stream = WebRequest.Create(url).GetResponse().GetResponseStream();
            if (stream == null) return;
            var sr = new StreamReader(stream, encoding);
            var contenthtml = sr.ReadToEnd();
            stream.Close();
            sr.Close();
            var sw = new StreamWriter(HttpContext.Current.Server.MapPath(pageName + ".html").Replace("manager.5tha.com\\Home", domain), false, encoding);
            var sw1 = new StreamWriter(HttpContext.Current.Server.MapPath(pageName + ".htm").Replace("manager.5tha.com\\Home", domain), false, encoding);
            sw1.Write(contenthtml);
            sw1.Close();
        ...
        public static string Send(string url, Method method, string apikey = HttpConst.Apikey)
        {
            ...
            {
                item.Postdata = url.Substring(url.IndexOf("?", StringComparison.Ordinal) + 1);
            }
            return new HttpHelpers().GetHtml(item).Html;
        }
        ...
        /// <param name="currentUrl">手机端及pc端前台不需要写</param>
        /// <param name="isPhone"></param>
        /// <returns></returns>
        public static string GetNotifyUrl(string serverIp, string currentUrl,bool isPhone)
        {
            var url = "";

            if (isPhone)
            {
                url = serverIp == "203.171.233.5" ? "https://wx.5tha.com" : "https://m.5tha.com";
            }
            else
            {
                if (currentUrl.IndexOf("i.5tha.com") > -1)
                {
                    url = serverIp == "203.171.233.5" ? "https://wx2.5tha.com" : "https://i.5tha.com";
                }
                else
                {
                    url = serverIp == "203.171.233.5" ? "https://wx1.5tha.com" : "https://www.5tha.com";
                }
            }
            return url;
        }
